...
class TrellisService:
    ...
    def generate_textures_for_shapes(
        ...
        # Determine adjusted steps based on voxel count ranges
        adjusted_slat_steps = self.settings.adaptive_step_ranges[-1][1]  # Default to last range
        for max_voxels, num_steps in self.settings.adaptive_step_ranges:
            if max_voxel_count <= max_voxels:
                adjusted_slat_steps = num_steps
                logger.info(f"Max voxel count {max_voxel_count} <= {max_voxels}: Using {num_steps} texture steps")
                break
        else:
            # If no range matched (voxel count > all thresholds), use last value
            logger.info(f"Max voxel count {max_voxel_count} > all thresholds: Using {adjusted_slat_steps} texture steps")
